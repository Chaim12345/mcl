...
import { BoardTableRowDnd } from '@/components/board/board-table-row-dnd';
...
import { DragDropProvider } from '@/components/board/dnd-provider';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
interface BoardTableDndProps {
  board: Board;
  onOpenItemDetail?: (itemId: string) => void;
}

export function BoardTableDnd({ board, onOpenItemDetail }: BoardTableDndProps) {
  ...
  const { data: items, isLoading: isLoadingItems } = useQuery({
    queryKey: ['boardItems', board.id],
    queryFn: () => boardService.getBoardItems(board.id),
    enabled: !!board.id,
  });

  // Initialize drag and drop functionality
  const { isDragging, setIsDragging, moveItem, handleDragEnd, handleStatusDrop } = useBoardDnd(board.id, items);
  ...
  const getTotalTableWidth = () => {
    ...
    // Add 20px for the selection checkbox column and drag handle
  ...
  return (
    <DragDropProvider>
      <div 
        ...
        style={{ cursor: isResizing ? 'col-resize' : isDragging ? 'grabbing' : 'default' }}
      >
        ...
        <ScrollArea className="h-[calc(100vh-220px)]">
          ...
          <div 
            style={{ 
              width: `${Math.max(tableWidth, getTotalTableWidth() + 20)}px`, // Add 20px for drag handle and selection columns
              height: '1px'
            }} 
          />
        </ScrollArea>
